...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # OPPO-specific: Prepare oracle log probs and success scores
        if adv_estimator == AdvantageEstimator.OPPO:
            # Oracle log probs: computed from prompts augmented with ground truth
            adv_kwargs["oracle_log_probs"] = data.batch["oracle_log_probs"]
            # Optional: Oracle R=0 log probs (prompt augmented with a negative answer)
            # If present, OPPO advantage will use this as π(y|R=0, x, y*wrong) instead of std prompt likelihood.
            if "oracle0_log_probs" in data.batch:
                adv_kwargs["oracle0_log_probs"] = data.batch["oracle0_log_probs"]
            # Optional: validity mask for oracle0 (per-sample). If absent, assume all valid.
            if "oracle0_valid" in data.batch:
                adv_kwargs["oracle0_valid"] = data.batch["oracle0_valid"]
            # Old log probs: computed from current policy on actual responses
            adv_kwargs["old_log_probs"] = data.batch["old_log_probs"]
            # Success scores: computed in rollout phase, used for R_bar calculation
            if "oppo_success" in data.batch:
                adv_kwargs["success_scores"] = data.batch["oppo_success"]

        if adv_estimator == AdvantageEstimator.HSD:
...
class RayPPOTrainer:
    ...
    def _build_oracle_prompt_batch(self, batch: DataProto, oracle_cfg=None) -> Optional[DataProto]:
        """
        Core Change 1: Oracle Prompt Construction
        
        Purpose: Let the model evaluate trajectories under "known correct answer" condition.
        
        Method: Inject ground truth y* into prompt in some form.
        
        Original: "Question: {query}\\nResponse:"
        Oracle:   "Question: {query}\\n[Answer: {y*}]\\nResponse:"
        
        This creates prompts that condition on ground truth, allowing computation of
        π_oracle(y_t | y_{1:t-1}, x, y*) - the probability of each token given the oracle context.
        
        Args:
            batch: DataProto containing prompts, responses, and ground truth.
            oracle_cfg: Optional config for oracle prompt construction. If None, uses
                self.config.algorithm.oppo. Allows HSD to pass its own config.
            
        Returns:
            DataProto with oracle-augmented prompts, or None if ground truth not available.
        """
        ...
        if oracle_cfg is None:
            oracle_cfg = self.config.algorithm.oppo
        truncation = oracle_cfg.get("truncation", None) or self.config.data.get("truncation", "left")
        ...
        for i in range(batch_size):
            ...
            if ground_truth is not None:
                ...
                oracle_prefix = oracle_cfg.get("oracle_prompt_prefix", "")
                oracle_suffix = oracle_cfg.get("oracle_prompt_suffix", "")
                oracle_sep = oracle_cfg.get("oracle_prompt_sep", " ")
                oracle_text = f"{oracle_prefix}{ground_truth}{oracle_suffix}"
            ...
            oracle_suffix_ids = (
                self.tokenizer.encode(oracle_text, add_special_tokens=False) if oracle_text else []
            )
            ...
            if merged_ids.numel() > prompt_len:
                if truncation == "left":
                    ...
                    raise RuntimeError(
                        f"Oracle prompt length {merged_ids.numel()} exceeds max {prompt_len}."
                    )
                else:
                    merged_ids = merged_ids[-prompt_len:]
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # ====================================================================
                    # Core Change 2: Oracle Forward Pass
                    # ====================================================================
                    # Purpose: Get π_oracle(y_t | y_{1:t-1}, x, y*) for each token
                    # Method: Perform an additional forward pass with Oracle Prompt on the
                    #         already sampled trajectory to get log prob for each token
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.OPPO:
                        # Step 1: Build oracle prompt batch (Core Change 1)
                        oracle_batch = self._build_oracle_prompt_batch(batch)
                        if oracle_batch is None:
                            raise RuntimeError("OPPO requires oracle prompt data, but none was found.")
                        
                        # Step 2: Forward pass with oracle prompt to get oracle log probs
                        # This computes π_oracle(y_t | y_{1:t-1}, x, y*) for each token position
                        oracle_log_prob = self.actor_rollout_wg.compute_log_prob(oracle_batch)
                        
                        # Clean up unnecessary fields
                        if "entropys" in oracle_log_prob.batch:
                            oracle_log_prob.batch.pop("entropys")
                        if "entropy" in oracle_log_prob.batch:
                            oracle_log_prob.batch.pop("entropy")
                        
                        # Rename to oracle_log_probs for clarity
                        oracle_log_prob.batch["oracle_log_probs"] = oracle_log_prob.batch.pop("old_log_probs")
                        batch = batch.union(oracle_log_prob)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # OPPO-specific: Compute success scores for R_bar calculation
                        # OPPO expects values in [0, 1]; clamped inside the advantage fn.
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.OPPO:
                            if "acc" in batch.non_tensor_batch:
                                ...
                                # Fallback: min-max normalize reward sums to [0, 1]
                                reward_sum = batch.batch["token_level_scores"].sum(dim=-1)
                                r_min, r_max = reward_sum.min(), reward_sum.max()
                                if r_max > r_min:
                                    success_scores = (reward_sum - r_min) / (r_max - r_min)
                                else:
                                    success_scores = torch.full_like(reward_sum, 0.5)
                            batch.batch["oppo_success"] = success_scores
